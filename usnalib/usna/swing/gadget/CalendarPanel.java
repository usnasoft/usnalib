...
import usna.swing.IntegerTextField;
...
public class CalendarPanel extends JPanel {
	...
	private IntegerTextField anno = null;
		...
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				...
				for (int i = 1; i <= today.getActualMaximum(GregorianCalendar.DAY_OF_MONTH); i++) {
					...
					dayBut.addActionListener(new ActionListener() {
	...
	private JComboBox<String> getComboMonths() {
		if (comboMonths == null) {
			...
			comboMonths.addActionListener(new java.awt.event.ActionListener() {
	 ...
	 */
	private IntegerTextField getAnno() {
		if (anno == null) {
			anno = new IntegerTextField(1, 1, 9999);
			anno.setColumns(4);
	...
	private JButton getAnnoMeno() {
		if (annoMeno == null) {
			final ImageIcon icon = new ImageIcon(CalendarPanel.class.getResource("left.gif"));
			annoMeno = getAnno().getArrowDown(icon);
	...
	private JButton getAnnoPiu() {
		if (annoPiu == null) {
			final ImageIcon icon = new ImageIcon(CalendarPanel.class.getResource("right.gif"));
			annoPiu = getAnno().getArrowUp(icon);
